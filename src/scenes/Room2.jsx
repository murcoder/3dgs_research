import { Floor } from '../models/Floor.jsx';
import Player from '../models/Player.jsx';
import { Physics } from '@react-three/rapier';
import React, { forwardRef } from 'react';
import { CameraControls, Splat } from '@react-three/drei';
import { Lasercutter } from '../boundries/Lasercutter.jsx';
import { Door } from '../boundries/Door.jsx';

const Room2 = forwardRef(({debug, laserCutterClicked, cameraMode, doorClicked}, ref) => {

  return (
    <group ref={ref}>
      <Physics debug={debug} timeStep="vary">
        <Splat scale={1.3} src="./splats/garage.splat" />
        <Lasercutter position={{x:-0.6,y:0.9,z:1.7}} boxGeometry={{width:1.2,height:1.6,depth:2.7}} onMachineClick={laserCutterClicked} />
        <Door position={{x:4.2, y:1.54, z:-2.02}} rotation={{x:0,y:1.57,z:0}} onDoorClick={doorClicked}/>
        <Floor />
        {cameraMode === 'orbit' ? (
          <CameraControls />
        ) : (
          <Player position={[2.3, 1, -1.5]} cameraPos={{ x: 0, y: 30 }} />
        )}
      </Physics>
    </group>
  );
});

export default Room2;
